...
{	printf("\t\tInizioprog_iterativo_da_microcontrollore\n");
	...
	float lay_1[8][SIZE_L1_2][SIZE_L1_2];
		...
		for (i = 0; i < 64; i=i+8)
		{
			for(h=0; h<8; h++)
			{
				for (j = 0; j < CHANNEL_SIZE; j++)
				{
					...
					mf1++;
					convolution2D(SIZE_L1_2, input_int[j], kern.kernel_9, &lay_1[h][0][0]);
						printf("Layer_1: conv %d*%d alla riga %d\n",(j+1),(j+1),(i+h+1));
				}
					printf("Terminate le 3 conv di Layer_1 di riga:%d\n",(i+h+1));
				bias = getBias(ptb1,mb1);
				mb1++;
				addBias(SIZE_L1_2, &lay_1[h][0][0], bias);
				relu(SIZE_L1_2, &lay_1[h][0][0]);
			
				getWeights(ptf2, 9, kern.kernel_9, mf2);
				mf2++;
				convolution2D(SIZE_L1_2, &lay_1[h][0],  kern.kernel_9, &lay_2[0][0]);
					printf("Layer_2: conv %d*%d alla riga %d\n",(i+1),(i+1),(k+1));
				matrixReset(SIZE_L1_2, &lay_1[h][0][0]);
			}
		}
		printf("Terminate le 64 conv di Layer_2 di riga:%d\n",(j+1));
		// Effettua la maxpool sulla matrice ottenuta in uscita dal layer2, e salvane il risultato in odds.lay_3 //
