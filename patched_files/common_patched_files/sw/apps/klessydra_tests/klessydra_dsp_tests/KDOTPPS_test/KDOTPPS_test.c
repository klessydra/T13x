...
{	
	...
	// TEST KDOTPPS8 -----------------------------------------------------------------------------	
    res8=kless_post_scal_dot_product_8((void*) result8, (void*) vect8_1, (void*) vect8_2, (void*) p_scal8, size8);
	//------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		for (int i=0; i<NumOfElements; i++)
		{
			testres8 += (vect8_1[i]*vect8_2[i]) >> p_scal8;
		}
		__asm__("csrrw zero, 0x7A0, 0x00000000;"
	...
	// TEST KDOTPPS16 ----------------------------------------------------------------------------	
	res16=kless_post_scal_dot_product_16((void*) result16, (void*) vect16_1, (void*) vect16_2, (void*) p_scal16, size16);
	//------------------------------------------------------------------------------------------
	// DISABLE COUNTING AND SAVE MCYCLE OF EACH THREAD ////////
	__asm__("csrrw zero, 0x7A0, 0x00000000;"
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		for (int i=0; i<NumOfElements; i++)
		{
			testres16 += ((vect16_1[i]*vect16_2[i]) >> p_scal16);     
		}
		__asm__("csrrw zero, 0x7A0, 0x00000000;"
			"csrrw %[test_perf_i], mcycle, zero;"
			"sw %[test_perf_i], 0(%[test_ptr_perf]);"
	...
	// TEST KDOTPPS32 ----------------------------------------------------------------------------	
	res32=kless_post_scal_dot_product_32((int*) result32, (void*) vect32_1, (void*) vect32_2, (void*) p_scal32, size32);
	//------------------------------------------------------------------------------------------
