...
int main()
{
	...
	// TEST KSVADDRF8 ----------------------------------------------------------------------------	
	res8=kless_scalar_vect_add_rf_8((void*) result8, (void*) vect8_1, (void*) scal8_2,  size8);
	//------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		for (int i=0; i<NumOfElements; i++)
		{
			...
			{
				goto FAIL_VECT_SVADD_RF_8;
			}
		}
		if (add_pass==NumOfElements)
		{
			printf("\nPASSED KSVADDRF8  8-bit  scalar-vector addition");
		}
	}
	...
	// TEST KSVADD16 ---------------------------------------------------------------------------
	res16=kless_scalar_vect_add_rf_16((void*) result16, (void*) vect16_1, (void*) scal16_2, size16);
	//------------------------------------------------------------------------------------------
	// DISABLE COUNTING AND SAVE MCYCLE OF EACH THREAD ////////
	__asm__("csrrw zero, 0x7A0, 0x00000000;"
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		for (int i=0; i<NumOfElements; i++)
		{
			...
			{
				goto FAIL_VECT_SVADD_RF_16;
			}
		}
		if (add_pass==NumOfElements)
		{
			printf("\nPASSED KSVADDRF16 16-bit scalar-vector addition");
		}
	}
	// -----------------------------------------------------------------------------------------
	...
	// TEST KSVADDRF32 ---------------------------------------------------------------------------	
	res32=kless_scalar_vect_add_rf_32((void*) result32, (void*) vect32_1, (void*) scal32_2, size32);
	//------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		for (int i=0; i<NumOfElements; i++)
		{
			...
			{
				goto FAIL_VECT_SVADD_RF_32;
			}
		}
		if (add_pass==NumOfElements)
		{
			printf("\nPASSED KSVADDRF32 32-bit scalar-vector addition\n\n");
		}
	}
	// -----------------------------------------------------------------------------------------
	...
	{		
		...
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVADDRF8  Speed: %d Cycles\n",i, perf8[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVADDRF16 Speed: %d Cycles\n",i, perf16[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVADDRF32 Speed: %d Cycles\n",i, perf32[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("SVADDRF%d Speed: %d Cycles\n",8*(2^i), testperf[i]);
		}
		return 0;
	}	
	...
	FAIL_VECT_SVADD_RF_8: 
	printf("\nFAILED KSVADDRF8  8-bit scalar-vector addition");
	goto VECT_SVADD_RF_16;
	FAIL_VECT_SVADD_RF_16: 
	printf("\nFAILED KSVADDRF16 16-bit scalar-vector addition");
	goto VECT_SVADD_RF_32;
	FAIL_VECT_SVADD_RF_32: 
	printf("\nFAILED KSVADDRF32 32-bit scalar-vector addition\n");
	if (Klessydra_get_coreID()==1)
	{		
		printf("\nNumber of Elements: %d\n\n",NumOfElements);
	}
