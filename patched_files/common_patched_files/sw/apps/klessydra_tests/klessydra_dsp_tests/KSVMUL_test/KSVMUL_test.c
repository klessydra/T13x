...
int main()
{
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		{
			printf("\nPASSED KSVMUL8  8-bit  scalar-vector multiplication");
		}
	}
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		if (mult_pass==NumOfElements)
		{
			printf("\nPASSED KSVMUL16 16-bit scalar-vector multiplication");
		}
	}
	// -----------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		for (int i=0; i<NumOfElements; i++)
		{
			if (res32[i]==testres32[i])  // AAA change this if the result width in klessydra is ever changed
			{
				mult_pass++;
			}
			else 
		...
		if (mult_pass==NumOfElements)
		{
			printf("\nPASSED KSVMUL32 32-bit scalar-vector multiplication\n\n");
		}
	}
	// -----------------------------------------------------------------------------------------
	...
	FAIL_VECT_SVMUL_8: 
	printf("\nFAILED KSVMUL8  8-bit scalar-vector multiplication");
	goto VECT_SVMUL_16;
	FAIL_VECT_SVMUL_16: 
	printf("\nFAILED KSVMUL16 16-bit scalar-vector multiplication");
	goto VECT_SVMUL_32;
	FAIL_VECT_SVMUL_32: 
	printf("\nFAILED KSVMUL32 32-bit scalar-vector multiplication\n");
	if (Klessydra_get_coreID()==1)
	{		
		printf("\nNumber of Elements: %d\n\n",NumOfElements);
	}
