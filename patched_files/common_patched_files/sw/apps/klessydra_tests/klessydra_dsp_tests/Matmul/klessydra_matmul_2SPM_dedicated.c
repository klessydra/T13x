...
int main(){
	...
	for(int i=0;i<n;i++){
		k=0;
		p=0;
		kmemld((void *)((int *)spmaddrA+u), &m2[0][0],  SIZE_OF_INT*(u*scl));
		for(int j=0;j<m;j++){
			if(j%scl==0 & j!=0){
				p++;
				k=0;
				if(m-(p*scl)>scl){
					kmemld((void *)((int *)spmaddrA+u), &m2[j][0],  SIZE_OF_INT*(u*scl));
				}
				else{
					kmemld((void *)((int *)spmaddrA+u), &m2[j][0],  SIZE_OF_INT*((m-scl*p)*u));
				}
			}
			offset[1]=u+k*u;
