...
int m2[N_ROW_1][N_COL_1]={27, 28, 46, 1, 30, 38, 3, 10, 30, 22, 9, 36, 35, 14, 3, 27, 0, 5, 20, 47, 32, 8, 14, 22, 35, 37, 23, 18, 29, 11, 11, 9, 39, 7, 10, 19, 46, 16, 32, 28, 38, 41, 16, 25, 7, 19, 3, 7, 24, 25, 5, 7, 34, 19, 31, 19, 9, 4, 39, 40, 15, 1, 1, 7, 
...
int main(){
	...
	int u=N_COL_2;
	int k,p;
	int scl=(SPM_MAX*SPM_MAX-(3*u))/(3*u);
	int offset[3]={0,0,0};
	...
	if(Klessydra_get_coreID()==0){
		...
		for(int i=0;i<n;i++){
			k=0;
			p=0;
			kmemld((void *)((int *)spmaddrA+(3*u)), &m2[0][0],  SIZE_OF_INT*(u*scl));
			for(int j=0;j<m;j++){
				if(j%scl==0 & j!=0){
					p++;
					k=0;
					if(m-(p*scl)>scl){
						kmemld((void *)((int *)spmaddrA+(3*u)), &m2[j][0],  SIZE_OF_INT*(u*scl));
					}
					else{
						kmemld((void *)((int *)spmaddrA+(3*u)), &m2[j][0],  SIZE_OF_INT*((m-scl*p)*u));
					}
				}
				offset[1]=k*u+(3*u);
				ksvmulrf((void *)((int *)spmaddrB),(void *)((int *)spmaddrA+offset[1]),m1[i][j]);
				kaddv((void *)((int *)spmaddrA),(void *)((int *)spmaddrA),(void *)((int *)spmaddrB));
				k++;
			}
			kmemstr(&m_out[i][0],  (void *)((int *)spmaddrA),  u*SIZE_OF_INT);
			kbcast((void *)((int *)spmaddrA),(void *)zero);
		}
	}
	if(Klessydra_get_coreID()==1){
		...
		for(int i=0;i<n;i++){
			k=0;
			p=0;
			kmemld((void *)((int *)spmaddrA+u*scl+(3*u)), &m2[0][0],  SIZE_OF_INT*(u*scl));
			for(int j=0;j<m;j++){
				if(j%scl==0 & j!=0){
					p++;
					k=0;
					if(m-(p*scl)>scl){
						kmemld((void *)((int *)spmaddrA+u*scl+(3*u)), &m2[j][0],  SIZE_OF_INT*(u*scl));
					}
					else{
						kmemld((void *)((int *)spmaddrA+u*scl+(3*u)), &m2[j][0],  SIZE_OF_INT*((m-scl*p)*u));
					}
				}
				offset[1]=u*scl+k*u+(3*u);
				ksvmulrf((void *)((int *)spmaddrB+u),(void *)((int *)spmaddrA+offset[1]),m1[i][j]);
				kaddv((void *)((int *)spmaddrA+u),(void *)((int *)spmaddrA+u),(void *)((int *)spmaddrB+u));
				k++;
			}
			kmemstr(&m_out[i][0],  (void *)((int *)spmaddrA+u),  u*SIZE_OF_INT);
			kbcast((void *)((int *)spmaddrA+u),(void *)zero);
		}
	}
	if(Klessydra_get_coreID()==2){
		...
		for(int i=0;i<n;i++){
			k=0;
			p=0;
			kmemld((void *)((int *)spmaddrA+u*scl+u*scl+(3*u)), &m2[0][0],  SIZE_OF_INT*(u*scl));
			for(int j=0;j<m;j++){
				if(j%scl==0 & j!=0){
					p++;
					k=0;
					if(m-(p*scl)>scl){
						kmemld((void *)((int *)spmaddrA+u*scl+u*scl+(3*u)), &m2[j][0],  SIZE_OF_INT*(u*scl));
					}
					else{
						kmemld((void *)((int *)spmaddrA+u*scl+u*scl+(3*u)), &m2[j][0],  SIZE_OF_INT*((m-scl*p)*u));
					}
				}
				offset[1]=k*u+u*scl+u*scl+(3*u);
				ksvmulrf((void *)((int *)spmaddrB+u+u),(void *)((int *)spmaddrA+offset[1]),m1[i][j]);
				kaddv((void *)((int *)spmaddrA+u+u),(void *)((int *)spmaddrA+u+u),(void *)((int *)spmaddrB+u+u));
				k++;
			}
			kmemstr(&m_out[i][0],  (void *)((int *)spmaddrA+u+u),  u*SIZE_OF_INT);
			kbcast((void *)((int *)spmaddrA+u+u),(void *)zero);
		}
	}
	sync_barrier_reset();		
	sync_barrier_thread_registration();
