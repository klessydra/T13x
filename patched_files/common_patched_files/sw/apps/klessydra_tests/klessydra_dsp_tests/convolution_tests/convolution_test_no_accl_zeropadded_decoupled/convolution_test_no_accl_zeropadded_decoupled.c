		...
		for (int i=0; i<3; i++) {
			if (Klessydra_get_coreID() == i) {
				...
				// DISABLE COUNTING AND SAVE MCYCLE -------------------------------------------------------
					...
					final_perf0[i]=*(ptr_perf0);
					final_perf1[i]=*(ptr_perf1);
					final_perf2[i]=*(ptr_perf2);
					final_perf3[i]=*(ptr_perf3);
					final_perf4[i]=*(ptr_perf4);
					final_perf5[i]=*(ptr_perf5);
					final_perf6[i]=*(ptr_perf6);
					final_perf7[i]=*(ptr_perf7);
				#endif
				//------------------------------------------------------------------------------------------
			}
		}
