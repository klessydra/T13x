...
int main()
{
	...
	// TEST KSVSLT8 ----------------------------------------------------------------------------	
	res8=kless_scalar_vect_set_less_than((void*) result8, (void*) vect8_1, (void*) scal8_2, size8);
	//------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		...
		{
			printf("\nPASSED KSVSLT8  8-bit set vector less than (mask creation)");
		}
	}
	...
	// TEST KSVSLT16 ---------------------------------------------------------------------------
	res16=kless_scalar_vect_set_less_than((void*) result16, (void*) vect16_1, (void*) scal16_2, size16);
	//------------------------------------------------------------------------------------------
	// DISABLE COUNTING AND SAVE MCYCLE OF EACH THREAD ////////
	__asm__("csrrw zero, 0x7A0, 0x00000000;"
	...
	// Test 16-bit SVSLT result ----------------------------------------------------------------
	if (Klessydra_get_coreID()==1)
	{
		__asm__( "csrrw zero, 0x7A0, 0x00000001;");
		for (int i=0; i<NumOfElements; i++)
		{
			if (vect16_1[i] < scal16_2)
			{
				testres16[i] = 1;
			}
			else
		...
		*/
		...
		if (sra_pass==NumOfElements)
		{
			printf("\nPASSED KSVSLT16 16-bit set vector less than (mask creation)");
		}
	}
	// --------------------------------------------------------------------------------------------
	...
	// TEST KSVSLT -------------------------------------------------------------------------------	
	res32=kless_scalar_vect_set_less_than((void*) result32, (void*) vect32_1, (void*) scal32_2, size32);
	//--------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		...
		if (sra_pass==NumOfElements)
		{
			printf("\nPASSED KSVSLT32 32-bit set vector less than (mask creation)\n\n");
		}
	}
	// -------------------------------------------------------------------------------------------
	...
	{		
		...
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVSLT8  Speed: %d Cycles\n",i, perf8[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVSLT16 Speed: %d Cycles\n",i, perf16[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KSVSLT32 Speed: %d Cycles\n",i, perf32[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("SVSLT%d Speed: %d Cycles\n",8*power(2,i), testperf[i]);
		}
		return 0;
	}	
	...
	FAIL_SVSLT_8: 
	printf("\nFAILED KSVSLT8  8-bit set vector less than (mask creation)");
	goto SVSLT_16;
	FAIL_SVSLT_16: 
	printf("\nFAILED KSVSLT16 16-bit set vector less than (mask creation)");
	goto SVSLT_32;
	FAIL_SVSLT_32: 
	printf("\nFAILED KSVSLT32 32-bit set vector less than (mask creation)\n");
	if (Klessydra_get_coreID()==1)
	{		
		printf("\nNumber of Elements: %d\n\n",NumOfElements);
	}
