...
int main()
{
	...
	// TEST KVSLT8 ----------------------------------------------------------------------------	
	res8=kless_vector_set_less_than((void*) result8, (void*) vect8_1, (void*) vect8_2, size8);
	//------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		*/
		...
		{
			printf("\nPASSED KVSLT8  8-bit set vector less than (mask creation)");
		}
	}
	...
	// TEST KVSLT16 ---------------------------------------------------------------------------
	res16=kless_vector_set_less_than((void*) result16, (void*) vect16_1, (void*) vect16_2, size16);
	//------------------------------------------------------------------------------------------
	// DISABLE COUNTING AND SAVE MCYCLE OF EACH THREAD ////////
	__asm__("csrrw zero, 0x7A0, 0x00000000;"
	...
	// Test 16-bit VSLT result ----------------------------------------------------------------
	if (Klessydra_get_coreID()==1)
	{
		__asm__( "csrrw zero, 0x7A0, 0x00000001;");
		for (int i=0; i<NumOfElements; i++)
		{
			if (vect16_1[i] < vect16_2[i])
			{
				testres16[i] = 1;
			}
			else
		...
		for (int i=0; i<NumOfElements; i++)
		{
			printf("\nvect16_1: %x",vect16_1[i]);
			printf("\nvect16_2: %x",vect16_2[i]);
		}
		for (int i=0; i<NumOfElements; i++)
		{
		...
		*/
		...
		if (sra_pass==NumOfElements)
		{
			printf("\nPASSED KVSLT16 16-bit set vector less than (mask creation)");
		}
	}
	// --------------------------------------------------------------------------------------------
	...
	// TEST KVSLT -------------------------------------------------------------------------------	
	res32=kless_vector_set_less_than((void*) result32, (void*) vect32_1, (void*) vect32_2, size32);
	//--------------------------------------------------------------------------------------------
	...
	if (Klessydra_get_coreID()==1)
	{
		...
		for (int i=0; i<NumOfElements; i++)
		{
			printf("\nvect32_1: %x",vect32_1[i]);
			printf("\nsvect32_2: %x",vect32_2[i]);
		}
		for (int i=0; i<NumOfElements; i++)
		{
		...
		*/
		...
		if (sra_pass==NumOfElements)
		{
			printf("\nPASSED KVSLT32 32-bit set vector less than (mask creation)\n\n");
		}
	}
	// -------------------------------------------------------------------------------------------
	...
	{		
		...
		for(int i=0;i<3;i++)
		{
				printf("Th%d KVSLT8  Speed: %d Cycles\n",i, perf8[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KVSLT16 Speed: %d Cycles\n",i, perf16[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("Th%d KVSLT32 Speed: %d Cycles\n",i, perf32[i]);
		}
		for(int i=0;i<3;i++)
		{
				printf("VSLT%d Speed: %d Cycles\n",8*power(2,i), testperf[i]);
		}
		return 0;
	}	
	...
	FAIL_VSLT_8: 
	printf("\nFAILED KVSLT8  8-bit set vector less than (mask creation)");
	goto VSLT_16;
	FAIL_VSLT_16: 
	printf("\nFAILED KVSLT16 16-bit set vector less than (mask creation)");
	goto VSLT_32;
	FAIL_VSLT_32: 
	printf("\nFAILED KVSLT32 32-bit set vector less than (mask creation)\n");
	if (Klessydra_get_coreID()==1)
	{		
		printf("\nNumber of Elements: %d\n\n",NumOfElements);
	}
